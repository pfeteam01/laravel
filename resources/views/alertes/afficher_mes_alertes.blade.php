...
<div style="background-color: #9561e2">
    ...
    <a href="{{url('/affichermodifalerte/'.$mesAlertes->get($i)->id_alerte)}}" class="btn btn-light">Modifier l'annonce l'annonce</a>
    <a href="{{url('/changeretat/'.$mesAlertes->get($i)->id_alerte)}}" class="btn btn-light">@if($mesAlertes->get($i)->etat_alerte == 0){{'Activer de l\'annonce'}}@else {{'Désactiver l\'annonce'}} @endif</a>
    <a href="{{url('/supprimeralerte/'.$mesAlertes->get($i)->id_alerte)}}" class="btn btn-light">Supprimer l'annonce</a>
</div>
<br><br>
    @endfor
@stop
